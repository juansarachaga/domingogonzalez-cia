...
>
  <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center mt-20 mb-20">
    ...
    <div className="text-center md:text-left px-4 sm:px-0">
      <p className="text-[#AE0C21] text-[20px] font-semibold tracking-wide mb-2">
        NUESTROS LOGROS
      </p>
    ...
    <div className="flex flex-col md:flex-row justify-center md:justify-end items-center gap-8 md:gap-12 mt-6 md:mt-0 text-center md:text-right">
      <div>
        <h3 className="text-[50px] sm:text-[70px] md:text-[80px] font-semibold text-[#AE0C21] leading-[100%]">
          70+
        </h3>
      ...
      <div className="hidden md:block border-l-2 border-gray-300 h-16"></div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-6 text-center mb-20">
          {servicios.map((s) => (
            <a
              key={s.id}
              href={`#${s.id}`}
              className="max-w-xs mx-auto p-6 rounded-md border border-transparent 
                         transition duration-300 md:hover:shadow-lg md:hover:border-[#AE0C21] 
                         group min-h-[220px] flex flex-col"
            >
              <Image
                src={s.img}
                alt={s.nombre}
              ...
              <p className="text-sm sm:text-base text-gray-700 leading-snug flex-1">
                {s.texto}
              </p>
            </a>
          ))}
